...
// Bonding/cifratura completati: il client ANCS aspetta questo prima di iscriversi.
volatile bool g_ble_auth_done = false;
void ble_auth_set_done(bool v) { g_ble_auth_done = v; }
bool __attribute__((noinline)) ble_auth_is_done() { return g_ble_auth_done; }
...
public:
	...
	void onConnect(BLEServer *pServer, esp_ble_gatts_cb_param_t *param)
	{
		...
		memcpy(instance->peerAddr, param->connect.remote_bda, 6);   // per suspend()
		// Avvia subito cifratura+bonding MITM dal lato peripheral: senza questo iOS non fa
		// il pairing, l'ANCS client non si iscrive e iOS chiude la connessione dopo ~10s.
	...
	{
		g_ble_auth_done = false;   // reset per il prossimo ciclo di pairing
...
void BLENotifications::suspend()
{
	ESP_LOGI(LOG_TAG, "suspend()");
	if (isAdvertising)
	{
		BLEDevice::getAdvertising()->stop();
		isAdvertising = false;
	}
	// Chiudi il link con l'iPhone (se c'è): libera la connessione del controller
	// e la RAM del task ANCS, altrimenti il dual-role con l'OBD esaurisce la heap.
	esp_ble_gap_disconnect(peerAddr);
}
...
void BLENotifications::startAdvertising()
{
	...
	oAdvertisementData.setFlags(0x06);   // General Discoverable + BR/EDR Not Supported
	                                     // (0x01 = limited discoverable: iOS smette di vederlo dopo ~30s)
	...
	// Set security — MITM + bond (passkey-display), coerente anche in riconnessione
...
void setServiceSolicitation(BLEAdvertisementData &advertisementData, BLEUUID uuid)
{
	...
	case 16:
	{
		...
		// arduino-esp32 2.0.17: addData() vuole std::string (non String/StringSumHelper)
