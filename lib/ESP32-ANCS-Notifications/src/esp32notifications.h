...
public:
    ...
    /**
     * Chiude il link con l'iPhone e ferma l'advertising, senza fare il deinit
     * totale di stop(). Serve a liberare la radio/RAM BLE quando un'altra
     * funzione (es. client OBD) deve usare il Bluetooth. Per tornare operativi
     * basta richiamare startAdvertising(): l'iPhone si riaggancia col bond.
     */
    void suspend();
...
private:
    ...
    bool isAdvertising;
    uint8_t peerAddr[6];   // indirizzo dell'iPhone connesso, per poterlo disconnettere in suspend()
